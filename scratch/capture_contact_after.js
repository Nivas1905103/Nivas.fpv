...
async function captureContactAfter() {
  ...
  try {
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "contact_after_desktop.png"), Buffer.from(desktopShot.data, "base64"));

    // 2. Tablet Capture (820 x 1180)
    console.log("Capturing Tablet Contact...");
    await sendCommand(ws, msgId++, "Emulation.setDeviceMetricsOverride", {
      width: 820,
      height: 1180,
      deviceScaleFactor: 2,
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "contact_after_tablet.png"), Buffer.from(tabletShot.data, "base64"));
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "contact_after_mobile.png"), Buffer.from(mobileShot.data, "base64"));
...
captureContactAfter().catch(console.error);
