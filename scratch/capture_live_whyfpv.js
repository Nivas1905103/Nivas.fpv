...
async function captureLiveWhyFPV() {
  ...
  try {
    ...
    // 1. Live Desktop Capture (1440 x 1080)
    console.log("Capturing Live Desktop WhyFPV...");
    ...
    await sendCommand(ws, msgId++, "Page.navigate", { url: "https://nivasfpv.in/#why-fpv" });
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "live_whyfpv_redesign_desktop.png"), Buffer.from(desktopShot.data, "base64"));

    // Scroll to bento cards + CTA
    await sendCommand(ws, msgId++, "Runtime.evaluate", {
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "live_whyfpv_redesign_bento.png"), Buffer.from(desktopBentoShot.data, "base64"));

    // 2. Live Tablet Capture (820 x 1180)
    console.log("Capturing Live Tablet WhyFPV...");
    await sendCommand(ws, msgId++, "Emulation.setDeviceMetricsOverride", {
      width: 820,
      height: 1180,
      deviceScaleFactor: 2,
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "live_whyfpv_redesign_tablet.png"), Buffer.from(tabletShot.data, "base64"));

    // 3. Live Mobile Capture (390 x 844)
    console.log("Capturing Live Mobile WhyFPV...");
    await sendCommand(ws, msgId++, "Emulation.setDeviceMetricsOverride", {
      width: 390,
      height: 844,
      deviceScaleFactor: 3,
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "live_whyfpv_redesign_mobile.png"), Buffer.from(mobileShot.data, "base64"));
...
captureLiveWhyFPV().catch(console.error);
