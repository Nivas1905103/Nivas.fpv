...
async function captureAboutBefore() {
  ...
  try {
    const targets = await getJson("http://127.0.0.1:9238/json/list");
    ...
    console.log("Capturing Desktop About Before...");
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "about_before_desktop.png"), Buffer.from(desktopShot.data, "base64"));

    // 2. Mobile Capture (390 x 844)
    console.log("Capturing Mobile About Before...");
    await sendCommand(ws, msgId++, "Emulation.setDeviceMetricsOverride", {
      width: 390,
      height: 844,
      deviceScaleFactor: 3,
    ...
    fs.writeFileSync(path.join(ARTIFACTS_DIR, "about_before_mobile.png"), Buffer.from(mobileShot.data, "base64"));
...
captureAboutBefore().catch(console.error);
